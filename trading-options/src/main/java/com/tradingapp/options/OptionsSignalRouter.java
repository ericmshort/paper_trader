...
public class OptionsSignalRouter implements OptionsEvaluator {
    ...
    // Symbols stopped out this session — no re-entry until next trading day
    private final Set<String> sessionStopLossed = new HashSet<>();
    private LocalDate stopLossResetDate;
    // Supplied by TradingLoop; when true (bull market) we skip put entries
    ...
    public void evaluate(String symbol, double price, int buySignals, int sellSignals, String signalStr, String featureCsv) {
        ...
        if (opts.containsKey(callKey)) {
            ...
            if (sellSignals >= 2 || pos.daysToExpiry() < 3 || premiumStop || profitTarget) {
                ...
                if (premiumStop) sessionStopLossed.add(callKey);
        ...
        if (opts.containsKey(putKey)) {
            ...
            if (buySignals >= 2 || pos.daysToExpiry() < 3 || premiumStop || profitTarget) {
                ...
                if (premiumStop) sessionStopLossed.add(putKey);
        ...
        if (buySignals >= 2 && !opts.containsKey(callKey)) {
            if (sessionStopLossed.contains(callKey)) {
                researchCallback.accept(symbol + " CALL skip: stop-loss cooldown (re-entry blocked for session)");
            } else if (account.getPositions().containsKey(symbol)) {
        ...
        if (sellSignals >= 2 && !opts.containsKey(putKey)) {
            if (sessionStopLossed.contains(putKey)) {
                researchCallback.accept(symbol + " PUT skip: stop-loss cooldown (re-entry blocked for session)");
            } else if (uptrendSupplier != null && uptrendSupplier.getAsBoolean()) {
