...
class SignalWeightTrainerTest {
    ...
    void returnsDefaultWeightsWhenFewerThanMinTrades() {
        ...
        for (int i = 0; i < 9; i++) {
            trades.add(new LabeledTrade(new FeatureVector(50, 0.1, 100, 1.0, 1.5), true));
        }
    ...
    @Test
    void weightsSumToFiveOnSufficientData() {
    ...
    private List<LabeledTrade> buildBiasedTrades(int count) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            trades.add(new LabeledTrade(new FeatureVector(rsi, 0.1, 100, 1.0, 1.5), win));
        }
        return trades;
    }
}
